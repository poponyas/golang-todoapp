...
import (
	...
	core_http_types "github.com/poponyas/golang-todoapp/internal/core/transport/http/types"
)
...
// @Description Изменение задачи с конкретным ID
// @Description ### Логика обновления полей (Three-state logic):
// @Description 1. **Поле не передано**: `description` игнорируется. значение в БД не меняется
// @Description 2. **Явно передано значение**: `"description": "Выйти утром"`- устанавливает новое описание
...
// @Success 200 {object} PatchTaskResponse "Успешно измененный пользователь"
...
func (h *TasksHTTPHandler) PatchTask(rw http.ResponseWriter, r *http.Request) {
	...
	if err := core_http_request.DecodeAndValidateRequest(r, &request); err != nil {
		responseHandler.ErrorResponse(
			err,
			"failed to decode and validate patch user",
		)
